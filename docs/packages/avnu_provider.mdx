...
With these provider you (as sponsor) will be able to pay your users' gas fees and you (as a user) will be able to execute gasless transactions sponsored by the sponsor. So, the user will simply sign a typed data containing the calls he wants to execute. The gas fees in ETH, will be 
paid by Avnu relayers to execute the transaction. The process is as follows:

1.  As Sponsor, take contact with Avnu admins to get an API key and add credits to your account to be able to pay for the gas fees of your users. As mentioned in [Avnu docs](https://doc.avnu.fi/avnu-paymaster/cover-your-users-gas-fees)
2.  Then, add rewards to an account address (using `setAccountRewards` function) setting how many transactions you want to cover for that user, which contracts and functions he will be able to execute and expiration date of the rewards.
3.  Notify your users you have added rewards to their account.
4.  (Optional) User could check at any time how many transactions are left for his account (using `getAccountRewards` function).
5.  User prepare their transaction (using `buildTypedData` function).
6.  And finally, sign it (using `execute` function) and send it to the Avnu relayer.
7.  The relayer will execute the transaction, pay the gas fees and the sponsor will be charged for the gas fees.
8.  (Optional) The sponsor could check their rewards status (using `getSponsorActivity` function) and monitor the rewards usage.

Another use case is, for example, to use the AVNU Paymaster to receive signed transactions from your users and gas in any token to execute them and pay the corresponding gas in STRK or ETH to the sequencer.
...
- `deploymentData`: The deployment data. At this moment, this field is not used and is only provided for future compatibility with gasless account deployment.
...
You could check more examples in the packages [test](https://github.com/focustree/starknet.dart/packages/avnu_provider/test) folder.
...
### As a User
As a user, we set our transaction that we want to be sponsored. In the following example, we set an approve transaction to the ERC20 contract.
...
Now, we set other parameters to get our builded typed data from Avnu API. If we want that our transaction be sponsored, don't set the gasTokenAddress and maxGasTokenAmount fields
...
``` 
In the background, this is an example of asuccesful API response:
...
Once received the typed data, we need to remove null fields and generate the signature.
...
This is an example of succesful API response:
